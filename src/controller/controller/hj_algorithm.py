...
class HJController(ControllerBackend):
    ...
    def __init__(self, config):
        ...
        self._goal = np.asarray(config.get("goal", cfg.get("goal", [7.5, 0.0, 0.0])), dtype=float)
        self._corridor_params = {
        "x_knots": np.asarray(cfg.get("x_knots", DEFAULT_CORRIDOR["x_knots"]), dtype=float),
        "y_low_knots": np.asarray(cfg.get("y_low_knots", DEFAULT_CORRIDOR["y_low_knots"]), dtype=float),
        "y_high_knots": np.asarray(cfg.get("y_high_knots", DEFAULT_CORRIDOR["y_high_knots"]), dtype=float),
        }
        # STUDENT CODE END
    ...
    def get_action(
        ...
        # STUDENT CODE START
        u_nom = np.asarray(traj.actions[0], dtype=float).reshape(2) if traj is not None and traj.actions.shape[0] > 0 else np.zeros(2)
